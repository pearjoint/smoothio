...
,	"config_dns_p"		:	"Read *me* first:"
,	"h_config_dns_p"	:	"Manual pre-resolving of server host names, bypassing DNS:\n\ntypically only required on Windows machines; no need to or benefit from enabling this otherwise. Manually mapping your local machine name as well as 'localhost' to 127.0.0.1 is required in order to properly start up the Servers defined for this Instance, because whenever your machine does not have access to the DNS servers configured under ./_core/bin/windows/etc/resolv.conf (ie. when Windows is offline) not even your machine name or 'localhost' will be properly resolved and your Instance Servers cannot be started.\n\nEach line contains a mapping, consisting of a host name and its IP address, separated by a colon, such as mymachine:127.0.0.1\n\nAs a matter of course, only host names and IP addresses assigned to this machine will work at all."
...
,	"config_rs_fcl"		:	"restart only if uptime (in seconds) exceeds"
,	"config_unsaved"	:	"Configuration modifications <b>not yet saved</b>. <a href=\"#\" onclick=\"smio.configSaveChanges();\">Save now</a> (or <a href=\"#\" onclick=\"smio.refreshUI(false, false, true);\">Reload</a> the present configuration)."
